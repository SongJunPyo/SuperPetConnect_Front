...
## [해결됨] 2차 최종 헌혈 승인 시 사용자 알림

### 문제 상황 (해결됨)
서버에서는 `donation_completed` 타입으로 사용자/병원 모두에게 알림을 보내고 있었으나,
프론트엔드에서 잘못된 매핑으로 사용자 알림이 무시되고 있었습니다.

### 수정 내용 (프론트엔드)
`donation_completed` 매핑에 사용자, 병원 타입 추가 완료:

```dart
// notification_mapping.dart
'donation_completed': {
  UserType.admin: AdminNotificationType.donationCompleted,
  UserType.hospital: HospitalNotificationType.donationCompleted,  // 추가
  UserType.user: UserNotificationType.donationCompleted,          // 추가
},
```
...
| 대상 | 알림 타입 | 상태 |
|------|----------|------|
| 관리자 | `donation_completed` | ✅ |
| 병원 | `donation_completed` | ✅ |
| 사용자 | `donation_completed` | ✅ (수정 완료) |
