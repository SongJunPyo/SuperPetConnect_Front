...
class _ActiveUserBottomSheetState extends State<ActiveUserBottomSheet>
  ...
  /// AppliedDonationStatus 미러 (CLAUDE.md "신청 상태" 섹션):
  /// 0=대기 / 1=승인 / 3=완료. 2(완료대기)와 4(종결)는 admin 검토 흐름 외부라
  /// 이 필터에서는 표시하지 않음. 사용자 자발 취소는 hard delete로 행 자체가 사라짐.
                  ...
                  Expanded(
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          // 기본 정보 — 아이콘은 PetFieldIcons 단일 진실에서 가져옴.
                          _postDetailRow(PetFieldIcons.hospital, '병원명', postData.hospitalNickname ?? postData.hospitalName),
                          const SizedBox(height: 8),
                          _postDetailRow(PetFieldIcons.postLocation, '주소', postData.location),
                          const SizedBox(height: 8),
                          _postDetailRow(PetFieldIcons.species, '동물 종류', postData.animalType == 0 ? '강아지' : '고양이'),
                          const SizedBox(height: 8),
                          if (postData.bloodType != null && postData.bloodType!.isNotEmpty) ...[
                            _postDetailRow(PetFieldIcons.bloodType, '혈액형', postData.bloodType!),
                            const SizedBox(height: 8),
                          ],
                          _postDetailRow(PetFieldIcons.postedAt, '게시일', postData.createdDate.toString().split(' ')[0]),
                          // 헌혈 날짜
                          if (postData.donationDate != null) ...[
                            const SizedBox(height: 8),
                            _postDetailRow(PetFieldIcons.donationDate, '헌혈 날짜', postData.donationDate.toString().split(' ')[0]),
                          ],
                          // 수혈환자 정보 (긴급)
                          if (postData.isUrgent) ...[
                            ...
                            if (postData.patientName != null)
                              _postDetailRow(PetFieldIcons.patientName, '환자명', postData.patientName!),
                            if (postData.breed != null) ...[
                              const SizedBox(height: 8),
                              _postDetailRow(PetFieldIcons.breed, '품종', postData.breed!),
                            ],
                            if (postData.age != null) ...[
                              const SizedBox(height: 8),
                              _postDetailRow(PetFieldIcons.birthDate, '나이', '${postData.age}'),
                            ],
                            if (postData.diagnosis != null) ...[
                              const SizedBox(height: 8),
                              _postDetailRow(PetFieldIcons.diagnosis, '진단', postData.diagnosis!),
                            ],
                          ],
                          // 시간대 정보
