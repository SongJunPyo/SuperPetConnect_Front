...
class HospitalPostService {
  ...
  static Future<List<PostTimeItem>> getPostTimes({
    ...
    try {
      ...
      final uri = Uri.parse('$baseUrl/api/hospital/post-times')
      ...
      if (response.statusCode == 200) {
        ...
        if (data is Map && data['post_times'] != null) {
          final postTimesList = (data['post_times'] as List)
  ...
  static Future<List<RejectedPost>> getRejectedPosts() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('$baseUrl/api/hospital/posts/rejected'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json; charset=UTF-8',
        },
      ...
      if (response.statusCode == 422) {
        debugPrint('422 error - trying alternative method with getPostTimes(postStatus=2)');
      ...
      if (response.statusCode == 200) {
        ...
        if (data is Map && data['posts'] != null) {
          final rejectedPostsList = (data['posts'] as List)
  ...
  static Future<List<RejectedPost>> getRejectedPostsViaPostTimes() async {
    try {
      debugPrint('Fetching rejected posts via post-times API with postStatus=2');

      // post_status=2 (모집거절)인 시간대 조회
      final postTimeItems = await getPostTimes(postStatus: 2);

      debugPrint('Found ${postTimeItems.length} time-slot items with postStatus=2');
