...
/// 병원 게시글 현황의 Tab 1(헌혈모집) 전용 위젯.
///
/// 본인 병원의 게시글 중 `status == 1`(APPROVED, 모집 중)만 표시. 데이터 소스/필터/
/// 페이지네이션 패턴은 [HospitalPendingPostsTab]과 동일하며 status 필터만 다름.
class HospitalActivePostsTab extends StatefulWidget {
  final String searchQuery;
  final DateTime? startDate;
  final DateTime? endDate;
  final void Function(UnifiedPostModel post) onTapPost;

  const HospitalActivePostsTab({
    super.key,
  ...
  @override
  State<HospitalActivePostsTab> createState() => HospitalActivePostsTabState();
}

class HospitalActivePostsTabState extends State<HospitalActivePostsTab> {
  ...
  @override
  void didUpdateWidget(covariant HospitalActivePostsTab oldWidget) {
  ...
  List<UnifiedPostModel> get _filteredPosts {
    var filtered = _allPosts.where((post) => post.status == 1).toList();
